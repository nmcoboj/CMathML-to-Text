...
class LanguageGenerator(object):
    ...
    def generate_sub_language(self, stack, idioma, logging=False):
        """
        ...
        :return:
        """
        self.replace_constants(stack, idioma)
        self.reorganize(stack)
        while 'apply' in stack:
            i, j, temp_stack, operator, arity, key_op = self.get_operator(stack, idioma)
    ...
    def get_operator(self, stack,idioma):
        """
        ...
        key_op = temp_stack[1]
        if key_op not in self.get_valids_operators_from_template(idioma):
            raise NotImplementedError('%s is not implemented' % key_op)
        config = self.get_language_template(idioma)
    ...
    def prepare_operator(key_op, operator, values):
        ...
        # adicionado para processar una integral
        elif key_op == 'int':
            i = values.index('bvar')
            operator = operator.replace('$bvar$', values[i + 1])
            values.remove(values[i + 1])
    ...
    def replace_constants(self, stack,idioma):
        config = self.get_language_template(idioma)
        constants = self.get_valid_constants_from_template(idioma)
    ...
    @staticmethod
    def get_language_template(idioma):
        # language1.template e o arquivo com as implemetacoes de la operacoes matematicas
        location_file = path.join(path.dirname(__file__), idioma)
        return ConfigObj(location_file)

    def get_valids_operators_from_template(self,idioma):
        config = self.get_language_template(idioma)
        return config[self.k_operators].keys()

    def get_valid_constants_from_template(self,idioma):
        config = self.get_language_template(idioma)
